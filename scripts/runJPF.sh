...
lib=`find ${subject_dir}/lib -name "*.jar"|xargs echo|sed 's/ /:/g'`
...
then
    ...
    then
        echo Invoking JPF
        ${jpf_root}/bin/jpf +classpath=${subject_dir}/source/$1:$lib \
            org.apache.log4j.TestThrowableStrRep 2>&1 | tee ${subject_dir}/outputs/log4j3JPF.log
    else 
        if test $2 = "random" 
        then
            echo Invoking JPF
		  ...
		  #expect -c "set timeout $TIMEOUT; set log_file ${subject_dir}/outputs/log4j3JPF${seed}.log; spawn \
                  #${jpf_root}/bin/jpf +classpath=${subject_dir}/source/$1:$lib \
                  #+cg.randomize_choices=path +cg.seed=${seed} \
                  #org.apache.log4j.TestThrowableStrRep 2>&1 |; log_file ${subject_dir}/outputs/log4j3JPF${seed}.log; expect; log_file" \
                  #run JPF without expect
                  #comment out the following 4 lines if using expect above
                ${jpf_root}/bin/jpf +classpath=${subject_dir}/source/$1:$lib \
                    +cg.randomize_choices=path +cg.seed=$seed \
                    org.apache.log4j.TestThrowableStrRep 2>&1 | \
                    tee ${subject_dir}/outputs/log4j3JPF${seed}.log 
                X=$((X+1))
